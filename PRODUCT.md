...
- Progress/checklist state persists in the browser via `localStorage`, scoped strictly to that data — the one deliberate exception to the app's otherwise session-only design. Language and theme remain session-only by original design (no persistence across reloads); this was a conscious, user-approved decision, not scope creep.
