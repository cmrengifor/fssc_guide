...
- **Backdrop:** full-screen `rgba(10,14,22,.45)` scrim, `cursor:zoom-out` — the exact dimming value `.sidebar-backdrop` already uses for its off-canvas drawer, kept theme-independent and literal rather than introducing a new custom property for a single reuse.
- **Modal:** centered, `max-width:min(1100px,92vw)`, image corners at the system's `--radius` (10px), `role="dialog"` + `aria-modal="true"` + `aria-label` set to the image's own alt text. A circular close button (`Surface` fill, `Border` outline) sits at the top-right corner, overlapping the image slightly like a badge.
