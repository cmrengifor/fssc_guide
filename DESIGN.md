...
- **Provider:** GitHub and Google, both offered side by side on the sign-in card as equal-weight `.btn` options (neither is `.btn-primary` — picking one login method over the other isn't this screen's job to suggest).
- **Allow-list:** `auth.ts`'s `signIn` callback checks `profile.email` (Google) and `profile.login` (GitHub) against `AUTH_ALLOWED_USERS` — a comma-separated env var of allowed emails/usernames, case-insensitive. Empty/unset means no restriction (any GitHub or Google account can sign in); populate it to lock the app down without touching code. A rejected sign-in redirects to `/signin?error=AccessDenied`, which renders a `.signin-error` banner. Set in `.env.local` for local dev, in Vercel's env var dashboard for production. Other options considered but not built: a GitHub org-membership check, a Google Workspace domain check (`profile.hd`) — either can be added to the same callback if the audience shifts from a fixed list to a whole org/domain.
...
- **Signed-in identity:** `AccountMenu` (`components/AccountMenu.tsx`) sits in `TopBar`'s `.topbar-right`, before the language/theme toggles — a circular avatar (the provider's profile photo, `referrerPolicy="no-referrer"` since Google's photo URLs reject a same-origin referrer; falls back to the email's first letter on a `--blue-soft` chip when there's no photo), the signed-in email, then `SignOutButton`, visually grouped with a right border divider. Renders nothing while `useSession()` has no user (matches the "no placeholder state" principle used elsewhere). Email hides at the mobile breakpoint (avatar + sign-out only) — same space-saving move as hiding `.brand-text`.
