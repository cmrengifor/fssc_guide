...
### Holds reference
A small, static reference page (`/holds`) expanding the glossary's `Hold` entry, which used to end in "(Definition pending expansion)" — that parenthetical is gone now that the expansion exists, replaced with a `.glossary-see-also` link pointing here. Entries render as `.glossary-item` cards (same shape as the Glossary page) grouped under two category headings (Procurement / Invoice Processing), with a `.pending-note` at the top signaling the page itself is still growing (more hold types, resolution steps) even though the current entries are real, finished content — not a placeholder. Reachable three ways: a flat sidebar link directly under Home (no region/priority sub-grouping — holds aren't organized by either), the global search index, and inline from any WI step/tip that mentions a hold code (see above).
