...
- **Provider:** GitHub and Google, both offered side by side on the sign-in card as equal-weight `.btn` options (neither is `.btn-primary` — picking one login method over the other isn't this screen's job to suggest). No allow-list is applied to either — **any** GitHub or Google account can sign in. That's a deliberate, temporary posture for an IT-review demo, not a real access policy. Before this goes out with real (even PII-redacted) operational content, add a real restriction inside `auth.ts`'s `signIn` callback — a GitHub org-membership check, a Google Workspace domain check (`profile.hd`), or an email allow-list.
